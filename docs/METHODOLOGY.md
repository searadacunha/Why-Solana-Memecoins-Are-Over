...
This chapter is the contract the rest of the repository is held to: what every term means as a
formula, which population each rate is computed on, which validation procedure a claim had to survive
to be published, and where the whole thing stops being valid.

It sits apart from the results on purpose. A result is only as good as the definition of its
...
Five minutes, in order: [§0 Conventions at a glance](#0-conventions-at-a-glance), then
...
In prose they are referred to as **W1 ... W9**, ordered by ubiquity. One editorial note, since the
inconsistency shows up in the files: base58 vanity prefixes are chosen by whoever
...
exclusion. The hypothesis tested is a choice between two mechanisms, not an answer to "who is behind
this": `H_unique` (one entity) versus `H_methode` (several distinct entities applying the same
method). Strict disjointness of address sets combined with identical geometry supports `H_methode`,
and nothing beyond it is claimed. No identity, no intent, no attribution to any person is asserted
anywhere.
...
are zero-SOL legs and 2 have non-positive token amounts **[R]**. The filter is part of the count and
is quoted with it.
...
targets: `t_buyable` (defined as "peak occurs after detection", which selects tokens that went up) and
...
**The attrition ledger.** Published in full: the gap between "files collected" and "rows analysed"
is where sample sizes go to die **[R]**:
...
The consequence is unflattering. On the canonical corpus the 196 tokens fall into 20 clusters of
sizes **67, 34, 31, 12, 9, 8, 6, 6, 6, 5, 2, 2, 1, 1, 1, 1, 1, 1, 1, 1**; one single day supplies
**70 of 196** rows. Kish's effective sample size over clusters is **5.5** **[R]**. The correct
reading of T1 is "negative on ~6 effective observations", not "negative on 196". That number is
printed in the results tables, not only here.
...
An asymmetry makes T1 easy: its result is negative on every cell. Sweeping more policies can only
make a uniformly negative result *harder* to obtain by chance, so no multiplicity correction is
required there. Corrections defend a winner, not a null.
...
Most of this repository has no holdout. The one claim that did get a forward test failed it.

**Available.** One filter (second-wave concentration, "<= 10 buyers at rebuy") reached
**93 % in-sample on n = 60**, survived permutation, survived Bonferroni, survived a holdout, then
returned **53 % on n = 30 forward**, with AUC falling from 0.21 to 0.45 **[N]**. That collapse is the
single most useful result in the project: in-sample robustness, including the standard battery, is no
evidence of out-of-sample validity on this data. Underlying data is not in this repository, so the
figures are quoted as testimony, not as a verified measurement.
...
decomposition also says something. The mirror caught the true trough 78 times out of 162 (48 %), and
none of 27 flow features improved that capture rate (best AUC 0.596), which makes it a timing problem
...
2. **Publish the sensitivity.** Every run prints hard / soft / strict-book side by side. It matters:
   ...
   average.
3. **Characterise missingness on three axes**: time, covariates, outcome. Here: 352/645 captures
   lost; a runs test gives 10 observed runs against 320.8 expected (p = 0.0002), so the losses are
   outages, not dropout; and they are outcome-neutral on an independent outcome source **[P]**. The
   loss costs power, not validity, and that had to be *measured* rather than assumed.
...
Both rules are published because the answer depends on which one is used. Per-address contributions
are printed too: removing W1 alone takes the component
...
The symmetric error is just as easy. Ubiquity is evidence of sharing, not proof of it: one address
classified as infrastructure on ubiquity alone turned out to be a launch operator sniping its own 51
tokens **[N]**. The exclusion list is therefore declared explicitly in the code and testable, not
inferred by threshold at runtime.
...
is. Where a bias helps the author's thesis it is treated as fatal; this one does not help it.

Separately, the 70-token sample behind the creation-slot signature is **outcome-selected**: frozen
tokens that later reached >= 500 k USD, not a random draw of launches (§1.2), so the figure is
...
- Peak timestamps come from the pump.fun API (one-second resolution) while `detect_ts` is a local
  clock. A few seconds of skew are possible, which is why T3 publishes **three** thresholds
...
- **Identity or intent.** Addresses are technical identifiers observed on a public ledger. No natural
  ...
  financier, plausible as it is, is not measurable here.
