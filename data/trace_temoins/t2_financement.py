...
Lit e1_buyers_<label>.json (--buyers), ecrit e2_funding_<label>.json (--out), reecrit apres chaque
portefeuille. Les paginations sont mises en cache dans cache_sigs/.

Pieges :
1. Pagination, le piege n.1. `getSignaturesForAddress` ne remonte que du present vers le passe, donc
   les portefeuilles les plus actifs sont les plus difficiles a remonter : le biais va
   systematiquement contre les bots de sniping, c'est-a-dire contre les acteurs importants. Trois
   drapeaux explicites par portefeuille : `genesis_reached`, `prebuy_window_reached`,
...
2. Mesure par delta de solde. Un financement livre par fermeture d'un compte wrappe ne produit aucun
   transfert systeme alors que les soldes bougent. On lit `accountData[].nativeBalanceChange`,
   jamais les seuls transferts.
...
def token_sold(tx, wallet):
    ...
    positifs, precisement le piege que le groupe temoin doit mesurer, autant l'eliminer a la source.
    """
