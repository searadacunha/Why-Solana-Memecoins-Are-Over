#!/usr/bin/env python3
"""Etape 2 : comment chaque premier acheteur a ete finance, et par qui.

Lit le JSON de l'etape 1, ecrit e2_funding_<label>.json, met la pagination en cache dans
cache_sigs/.

Deux mesures distinctes, qui ne portent pas sur la meme chose.
M1, financement de naissance : les premieres transactions de la vie du portefeuille. C'est la
    mesure du cas ODIN, quatre portefeuilles nes dans la meme transaction cinq jours avant le
    token. Elle n'a de sens que si la genese est atteinte.
M2, financement pre-achat : les entrees de fonds dans la fenetre qui precede immediatement le
    premier achat (21 jours par defaut). Un portefeuille ancien et tres actif peut avoir ete
    re-approvisionne juste avant l'operation sans etre ne pour elle. Cette mesure reste accessible
    meme quand la genese est hors de portee, tant que la pagination atteint la fenetre.

Les separer est impose par le piege nº1 : `getSignaturesForAddress` ne remonte que du present vers
le passe, donc les portefeuilles les plus actifs sont les plus difficiles a remonter et le biais va
contre les bots de sniping, c'est-a-dire contre les acteurs importants. Trois drapeaux explicites
par portefeuille : `genesis_reached`, `prebuy_window_reached`, `pages_paginated`. Une absence de
financement sur un portefeuille dont la fenetre n'est pas atteinte est un echec de mesure, jamais
un resultat negatif.

Mesure par delta de solde : un financement livre par fermeture d'un compte wrappe ne produit aucun
transfert systeme alors que les soldes bougent. On lit `accountData[].nativeBalanceChange`, jamais
les seuls transferts.

Usage : python3 etape2_financement.py --buyers e1_buyers_OPTIMUS.json
"""
