...
def get_transactions(signatures, chunk=100, pause=0.12, progress=None):
    ...
    c'est-a-dire le delta de solde deja calcule, la mesure qui capte les financements obfusques
    par fermeture de compte wrappe.
...
def all_signatures(addr, max_pages=2000, label="", verbose=True, stop_ts=None):
    ...
    for i in range(max_pages):
        ...
        # Arret par projection. Optimisation : on ne s'arrete que si le plafond `max_pages`
        # serait de toute facon depasse d'au moins 50 %. La frontiere de decision est donc
        # inchangee, seul le temps de pagination est economise. Le motif est rendu a l'appelant.
