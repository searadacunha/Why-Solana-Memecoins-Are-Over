...
Le piege : `getSignaturesForAddress` ne remonte que du present vers le passe, donc les
portefeuilles les plus actifs sont les plus difficiles a remonter et le biais va contre les bots de
sniping. On rend trois drapeaux par portefeuille : `genesis_reached`, `prebuy_window_reached`,
...
def token_sold(tx, wallet):
    ...
    Un bot de sniping encaisse des centaines de rentrees de SOL par jour qui sont des produits de
    vente, pas des financements. Les compter comme financements ferait apparaitre des
    « decoupages » partout : des ventes simultanees de deux bots sur la meme courbe produisent
    mecaniquement des montants voisins au meme instant. Generateur de faux positifs, ecarte a la
    source.
    """
...
def paginate_cached(w, prebuy_start):
    ...
    pour un bot). Seul un resultat couvrant la fenetre est mis en cache, un echec est toujours
    retente."""
